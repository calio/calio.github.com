...
## How the field evolved: major “families” of world-model approaches
...
### 6) Representation-predictive (non-generative) world models: JEPA / V-JEPA 2
...
| MuZero                 | abstract latent for planning             | often implicit                           | no                          | supervised losses on reward/value/policy over unrolled steps |
| JEPA                   | embedding/belief state                   | often implicit or small latent           | no (predict embeddings)     | embedding regression + anti-collapse regularization          |
| Diffusion/AR video     | transformer context / latent cache       | sampling noise/seed                      | yes                         | diffusion denoising loss or token NLL                        |
| Robotics latent-action | belief + (latent) action token           | both in action and transition            | sometimes                   | forward prediction + inverse/action losses                   |

---

## The cleanest “mathematical discriminator”: **what distribution do you train?**

A very useful way to unify all of this:

### A) Observation-likelihood models (generative)
...
### B) Representation-predictive models (non-generative)

They optimize:
[
\min_\theta \sum_t d\big(\hat y_{t+1}, y_{t+1}\big) + \mathcal{R}_{\text{anti-collapse}}
]
where (y_t) is an embedding target. (JEPA family)

### C) Task-equivalent models (planning-centric)

They optimize:
[
\min_\theta \sum_t \Big(
\ell_r(\hat r_t,r_t)+\ell_v(\hat v_t,v_t)+\ell_\pi(\hat\pi_t,\pi_t)
\Big)
]
(MuZero family)

That’s the shortest way to mathematically “compare schools”: **they differ primarily in what they treat as the supervised signal** (pixels/tokens vs embeddings vs value/reward/policy), and where they put stochasticity.

---

## One practical conclusion (useful for your intro)
...
[
s_{t+k+1} = \mathrm{Pred}*\theta(s*{t+k}, h_{t+k}, a_{t+k}, z_{t+k})
]
...
**Signature:** “model the world” by modeling (p(x)) (pixels/tokens), plus dynamics.
...
[
\min*{\theta,\phi}\sum_t d(\hat y_{t+1},\mathrm{StopGrad}(y_{t+1}))+\lambda,\mathcal{R}_{\text{anti-collapse}}
]

**Signature:** no (p(x\mid s)) decoder; collapse prevention becomes central.

---

### Family C — Task-equivalent / planning-centric world models (MuZero-style)

They do **not** try to predict (x) at all; they predict decision-relevant quantities:
[
...
* **Explicit stochastic observation**: sampling noise controls (\hat x) (diffusion/AR video)
...
[
a_t = \Pi(\mathcal{M}*\theta, s_t) \quad \text{(planner uses model } \mathcal{M}*\theta)
]
...
where (\hat r_{t+k}=R_\theta(s_{t+k},a_{t+k})) and (s_{t+k+1}=\mathrm{Pred}*\theta(s*{t+k},h_{t+k},a_{t+k},z_{t+k})).
...
This is the dimension that really differentiates “schools in practice.” You can define (\mathcal{M}_\theta) as the world model (Enc+Pred+optional heads).
...
* Policy learns from *imagined* rollouts:
  [
  s_{t+1}=\mathrm{Pred}*\theta(s_t,h_t,a_t,z_t),\quad a_t\sim\pi*\psi(\cdot\mid s_t)
  ]

Policy objective (generic actor-critic in latent rollouts):
[
\max_\psi \ \mathbb{E}*{\pi*\psi,\mathcal{M}*\theta}\Big[\sum*{k=0}^{H-1}\gamma^k \hat r_{t+k}\Big]
]
...
[
\mathcal{D}*{\text{sim}} = \text{Rollout}(\mathcal{M}*\theta,\ \pi_\psi)
]
[
\min_\psi\ \mathbb{E}*{(s,a)\sim \mathcal{D}*{\text{sim}}}\big[-\log \pi_\psi(a\mid s)\big]
]
...
\quad,\quad
\min_\psi\ \sum_t |a_t-\hat a_t|^2 \ \text{or}\ -\log \pi_\psi(a_t\mid s_t)
]
